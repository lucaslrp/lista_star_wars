...
const Home: React.FC<Props> = ({
  ...
  const renderListFooter = () => {
    if (list.length === 0 && peopleLoading === false) {
      return <Text style={styles.flatListFooter}> Sem resultados </Text>;
    } else {
      return next === null ? (
        <Text style={styles.flatListFooter}> Fim de resultados </Text>
      ) : (
        <Text style={styles.flatListFooter}> Carregando... </Text>
      );
    }
  };
